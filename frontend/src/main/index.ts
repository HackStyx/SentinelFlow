...
function parseScapyPacket(dump) {
  try {
    ...
    // Convert buffer to string and handle non-printable characters
    const dumpStr = Buffer.from(dump).toString('utf-8').replace(/[^\x20-\x7E\n]/g, '');
    ...
    return JSON.stringify({ error: "Failed to parse packet" });
  }
}
